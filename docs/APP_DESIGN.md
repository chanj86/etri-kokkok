...
- 코트 선택 후 게임 방식을 고른다: 복식 2:2(정원 4명, 기본값) 또는 단식 1:1(정원 2명).
- 모든 회원이 슬롯을 만들 수 있다.
- 회원은 열린 슬롯의 `게임 참여` 버튼으로 직접 자리를 선택한다.
- 회원이 아닌 게스트도 이름만 입력해 추가할 수 있다.
  게스트는 순환·전적·랭킹 집계에 포함되지 않으며, 모집 중에는 제외할 수 있다.
- 한 회원이 둘 이상의 열린/진행 중 슬롯에 동시에 참여할 수 없다.
- 팀 구성이 완성되면(단식 1:1, 복식 2:2) 슬롯을 시작할 수 있다.
