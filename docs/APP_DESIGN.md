...
2. `last_joined_cycle < current_cycle`인 회원만 새 슬롯에 들어갈 수 있다.
3. 참여가 확정되면 해당 회원의 `last_joined_cycle`을 현재 순환으로 변경한다.
4. 활성 참석자 모두가 현재 순환에서 한 번 참여하면 `current_cycle`을 1 증가시킨다.
5. 순환이 슬롯 도중 바뀔 수 있으므로, 인원이 4의 배수가 아니어도 남은 자리를 다음 순환 회원이 채울 수 있다.
6. 같은 슬롯에 이미 들어간 회원은 순환이 바뀌어도 그 슬롯에 다시 들어갈 수 없다.

예를 들어 참석자가 6명이면 `ABCD`, `EFAB`, `CDEF` 순으로 세 게임을 구성할 수 있어 모든 회원이 두 번씩 참여한다.
