...
- 다음 참석자는 바로 앞 사람의 예상 시작 시각에 15분을 더한다.
- 취소 또는 미루기가 발생하면 영향을 받는 모든 예상 시각을 서버 트랜잭션에서 다시 계산한다.
- 가입 직후 순서와 예상 시각을 인앱 메시지로 안내한다.
...
- 예상 시작 15분 전에 푸시를 한 번 전송한다.
- 등록 또는 재계산 시 이미 15분 이내라면 즉시 안내한다.
- 시간 변경으로 같은 예약에 중복 알림이 가지 않도록 알림 로그에 예약 ID와 예정 시각을 기록한다.
...
`game_days.current_cycle`과 `game_attendances.last_joined_cycle`을 사용한다. 순환은 강제가 아니라 권고다.

1. 참석 등록 시 `last_joined_cycle`은 현재 순환보다 작은 값으로 시작한다.
2. `last_joined_cycle < current_cycle`인 회원은 `차례` 상태로 표시된다.
3. 자기 차례가 아니어도 참여할 수 있다. 다만 아직 순환하지 않은 회원이 남아 있으면 "현재 N번째 게임 참여입니다. (현재 순환 M회)" 안내와 함께 재확인을 받는다.
4. 참여가 확정되면 해당 회원의 `last_joined_cycle`을 현재 순환으로 올린다.
5. 활성 참석자 모두가 현재 순환에서 한 번 참여하면 `current_cycle`을 1 증가시킨다.
6. 순환이 슬롯 도중 바뀔 수 있으므로, 인원이 4의 배수가 아니어도 남은 자리를 다음 순환 회원이 채울 수 있다.
7. 자동 배치는 공정성을 위해 순환 차례를 지키는 회원만 후보로 선택한다.
...
- 열린/진행 중 게임은 재확인 후 삭제할 수 있으며, 삭제 시 참여자의 순환 상태가 되돌아가 다른 게임에 참여할 수 있다.
...
- 15분 전 푸시 또는 권한 미허용 시 인앱 안내가 동작한다.
- 게임 참석, 수동 슬롯 참여, 순환 잠금, 자동 배치, 전적 입력이 동작한다.
